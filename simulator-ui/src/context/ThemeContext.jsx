...
export function ThemeProvider({ children }) {
    ...
    useEffect(() => {
        ...
        // For cross-port sync (5173 <-> 5174), poll cookie periodically
        // Since different ports are different origins, we use cookies as a shared mechanism
        const getSharedCookie = () => {
            const cookies = document.cookie.split(';');
            for (let cookie of cookies) {
                const [name, value] = cookie.trim().split('=');
                if (name === 'app-theme' && THEMES[value]) {
                    return value;
                }
            }
            return null;
        ...
        // Poll cookie periodically to sync across different ports
        const pollInterval = setInterval(() => {
            const cookieTheme = getSharedCookie();
